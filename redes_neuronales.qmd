...
Vamos a hacer una revisión de las redes neuronales para posteriormente poder abordar los diferentes tipos de redes neuronales que se utilizan en Deep Learning. Algunos de los avances más recientes en varios de los diferentes componentes que forman parte de las redes neuronales están recopilados en [@noor2025surveystateoftheartdeeplearning], también se puede consultar el libro **Deep Learning** que es uno de los recopilatorios a nivel teórico más importante [@Goodfellow-et-al-2016].
...
![Perceptrón](imagenes/capitulo1/Perceptron.png){fig-alt="Perceptrón"}

En esta neurona nos encontramos que se realizan los siguientes cálculos: $$ z = \sum_{i=1}^{n}w_ix_i+b$$ $$\hat{y} = \sigma (z)$$ donde $x_i$ representan los datos numéricos de entrada, $w_i$ son los pesos, $b_i$ es el sesgo (bias), $\delta$ es la función de activación y finalmente $\hat y$ es el dato de salida.
...
![Perceptrón Multicapa](imagenes/capitulo1/PerceptronMulticapa.png){fig-alt="Perceptrón Multicapa"}
...
$$ donde $a_i^{(l-1)}$ representan los datos de la neurona $i$ en la capa $l-1$ ( siendo $a_i^0=x_i$ los valores de entrada), $w_{i j}^{(l)}$ son los pesos en la capa $l$, $b_i^{(l)}$ es el sesgo (bias) en la capa $l$, $\delta^{(l)}$ es la función de activación en la capa $l$ (puede que cada capa tenga una función de activación diferente), $n_j$ es el número de neuronas de la capa anterior que conectan con la $j$ y finalmente $a_j^{(l)}$ es el dato de salida de la capa $l$. Es decir, en cada capa para calcular el nuevo valor necesitamos usar los valores de la capa anterior.
...
Básicamente tenemos 2 tipos de datos:
...
Los datos de salida son la representación numérica del resultado de pasar a través de la red neuronal los datos de entrada. Normalmente nos encontramos que en una red neuronal las variables de salida son:

-   un número (regresión)
-   una serie de números (regresión múltiple)
-   un dato binario (clasificación binaria)
-   una serie de datos binarios que representa una categoría de entre varias (clasificación múltiple)

### Arquitectura de red

Para la construcción de una red neuronal necesitamos definir la arquitectura de esa red. Esta arquitectura, si estamos pensando en una red neuronal densamente conectada, estará definida por la cantidad de **capas ocultas** y el **número de neuronas** que tenemos en cada capa. También se podrán tener capas que sus neuronas no conectan con todas las de la capa anterior.

Más adelante veremos que dependiendo del tipo de red neuronal podrá haber otro tipo de elementos en estas capas.

### Función de coste y pérdida

Otro de los elementos clave que tenemos que tener en cuenta a la hora de usar nuestra red neuronal son las **funciones de pérdida y funciones de coste (objetivo)** ver [@elharrouss2025lossfunctionsdeeplearning].
...
Las correspondientes **funciones de coste** que se usarían, estarían asociadas a todas las muestras que se estén entrenando o sus correspondientes batches, así como posibles términos asociados a la regularización para evitar el sobre-ajuste del entrenamiento. Es decir, la función de pérdida se calcula para cada muestra, y la función de coste es la media de todas las muestras.

Por ejemplo, para el **Error medio cuadrático** $\left(L_2\right)$ tendríamos el siguiente valor: $$
\mathcal{J}_{\text {MSE }}(y, \hat{y})=\frac{1}{m} \sum_{i=1}^m \mathcal{L}_{\text {MSE }}(y, \hat{y})=\frac{1}{m} \sum_{i=1}^m|| \hat{y}_i-y_i \|^2=\frac{1}{n} \sum_{i=1}^m \sum_{i=1}^n\left(\hat{y}_{j i}-y_{j i}\right)^2
$$

### Optimizador

El **Optimizador** es el **algoritmo** encargado de **minimizar la función de coste**, de forma que consigamos minimizar el error de los datos que devuelve nuestra red neuronal, respecto a los datos que tiene la muestra. Para entender los principales optimizadores de Primer y Segundo Orden ver [@kashyap2023surveydeeplearningoptimizers].

Hay que tener en cuenta que el problema de optimización de la función de coste por la naturaleza es un **Problema No Convexo** debido a la no linealidad de las funciones de activación y al producto de los valores de cada capa con la capa anterior. Esto supone un gran problema a la hora de optimizar la función de coste, ya que nos vamos a encontrar que este problema **no** tiene un **mínimo global**. De forma que los algoritmos que usaremos, que si tienen mínimos globales cuando estamos ante **Problemas Convexos**, van a ser muy dependientes de las condiciones iniciales (inicialización de pesos), con lo cual no van a ser un algoritmo heurístico sino que podríamos decir que es aleatorio. Para el estudio de la **Optimización Convexa** ver [@Boyd_Vandenberghe_2004]

El **Descenso del gradiente** es la versión más básica de los algoritmos que permiten el aprendizaje en la red neuronal haciendo el proceso de **backpropagation** (propagación hacia atrás) para corregir el error y acercarnos a nuestro mínimo.

Recordamos que el descenso del gradiente nos permitirá actualizar los parámetros de la red neuronal cada vez que demos una pasada hacia delante (**forwardpropagation**) con todos los datos de entrada, volviendo con una pasada hacia atrás (**backpropagation**) haciendo uso de la **regla de la cadena** para encontrar las derivadas en cada capa..

$$\mathrm{W}_{\mathrm{t}}=\mathrm{W}_{\mathrm{t}-1}-\alpha \nabla_{\mathrm{W_{t-1}}} \mathcal{J}(\mathrm{W_{t-1}})$$

donde $\mathcal{J}$ es la **función de coste**, $\alpha$ es el parámetro de **ratio de aprendizaje** que permite definir como de grandes se quiere que sean los pasos en el aprendizaje, $\nabla_{W_{t-1}}$ es el **gradiente** respecto a los pesos, y $\mathcal{J}$ es la **función de coste**.
...
$$\mathrm{W}_{\mathrm{t}}=\mathrm{W}_{\mathrm{t}-1}-\alpha \nabla_{\mathrm{W_{t-1}}} \mathcal{J}(\mathrm{W_{t-1}}, x(i),y(i))$$
...
$$\mathrm{W}_{\mathrm{t}}=\mathrm{W}_{\mathrm{t}-1}-\alpha \nabla_{\mathrm{W_{t-1}}} \mathcal{J}(\mathrm{W_{t-1}}, \mathrm{B}(\mathrm{i}))$$
...
**Learning rate decay** Podemos definir un valor de descenso del ratio de aprendizaje, de forma que normalmente al inicio de las iteraciones de la red neuronal los pasos serán más grandes, pero conforme nos acercamos a la solución optima deberemos dar pasos más pequeños para ajustarnos mejor.

$$\mathrm{W}_{\mathrm{t}}=\mathrm{W}_{\mathrm{t}-1}-\alpha_{\mathrm{t}} \nabla_{\mathrm{W_{t-1}}} \mathcal{J}\left(\mathrm{W}_{\mathrm{t}-1}\right)$$
...
$$ V_ {t}  =  \gamma   V_ {t-1}  +  \alpha  V_ {t-1} J(  W_ {t-1}  ,x,y)$$ $$ W_ {t} =  W_ {t-1}  -  V_ {t} $$
...
$$ V_ {t}  =  \gamma   V_ {t-1}  +  \alpha   V_ {t-1}  J(  V_ {t-1}  -  \gamma   V_ {t-1}  ,x,y) $$ $$ W_ {t}  =  W_ {t-1}  -  V_ {t}  $$
...
![Paso Binario](imagenes/capitulo1/SaltoBinario.png){fig-alt="Paso Binario"}

-   **Identidad**

$F(x)=x$

![Lineal](imagenes/capitulo1/Lineal.png){fig-alt="Lineal"}

-   **Sigmoide (Logística)**

$F(x)=\frac{1}{1+e^{-x}}$

![Sigmoide](imagenes/capitulo1/Sigmoide.png){fig-alt="Sigmoide"}
...
![SoftMax](imagenes/capitulo1/SoftMax.png)

-   **ReLu ( Rectified Linear Unit)** $\begin{aligned} & F(x)=\max (0, x) \text{ o de la forma}\\ & F(x)= \begin{cases}0 & \text { for } x \leq 0 \\ x & \text { for } x>0\end{cases} \end{aligned}$

    [![ReLU](imagenes/capitulo1/Relu.png)](ReLU)
...
-   **RReLU (Randomized Rectified Linear Unit)** $F(\alpha, x)= \begin{cases}\alpha x & \text { for } x<0 \\ x & \text { for } x \geq 0\end{cases}$
    ...
    [![LRelu](imagenes/capitulo1/LRelu.png)](LRelu)
...
![ELu](imagenes/capitulo1/Elu.png){fig-alt="ELu"}
...
Las técnicas de regularización nos permiten conseguir mejorar los problemas que tengamos por sobreajuste en el entrenamiento de nuestra red neuronal. Para ver más información sobre regularización [@kukacka2017regularizationdeeplearningtaxonomy].
...
Cuando empieza el entrenamiento de una red neuronal y tiene que realizar la primera pasada hacia delante de los datos, necesitamos que la red neuronal ya tenga asignados algún valor a los pesos. Este paso puede resultar importante ya que el valor inicial de los valores de la función que vamos a optimizar guiarán al algoritmo hasta la convergencia.
...
Cuando los datos entran a la red neuronal y se comienza a operar con ellos, se convierten en nuevos valores que han perdido esa propiedad de normalización. Lo que hacemos con la **normalización por lotes** (batch normalization), ver \@[@ioffe2015batchnormalizationacceleratingdeep], es que añadimos un paso extra para normalizar las salidas de las funciones de activación. Lo normal es que se aplicara la normalización con la media y la varianza de todo el bloque de entrenamiento en ese paso, pero normalmente estaremos trabajando por lotes y se calculará la media y varianza con ese lote de datos.
...
# Realizamos la "Standarización" restando la media y dividiendo por la desviación típica
