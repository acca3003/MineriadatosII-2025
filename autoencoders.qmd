...
Los **Autoencoders (AE)** son uno de los tipos de redes neuronales que caen dentro del ámbito del Deep Learning, en la que nos encontramos con un modelo de **aprendizaje no supervisado**. Ya se empezó a hablar de AE en la década de los 80 [@bourlandautoassociation], aunque es en estos últimos años donde más se está trabajando con ellos.

La arquitectura de un AE es una Red Neuronal Artificial que se encuentra dividida en dos partes, **encoder** y **decoder** [@Charte_2018], [@Goodfellow-et-al-2016Goodfellow-et-al-2016]. El **encoder** va a ser la parte de que va **codificar** o comprimir los datos de entrada, y el **decoder** será el encargado de **regenerar** de nuevo los datos en la salida. Esta estructura de codificación y decodificación normalmente le llevará a tener una estructura simétrica. El **AE** es entrenado para ser capaz de **reconstruir** los **datos** de **entrada** en la capa de salida, implementando una serie de **restricciones** (la reducción de elementos en las capas ocultas del encoder) que van a evitar que simplemente se copie la entrada en la salida.
...
Según [@Charte_2018] los AE se pueden clasificar según el **tipo de arquitectura de red** en:
...
Según lo visto de forma intuitiva vamos a tener el **encoder** $f(X)$ que será el encargado de codificar los datos de entrada y luego tendremos el **decoder** $g(H)$ que será el encargado de realizar la decodificación y conseguir acercarnos al dato original . Es decir intentamos conseguir $g(f(X))=\hat{Y} \approx X$ . Si suponemos un **Simple Autoencoder** en el que tenemos una única capa oculta, con una función de activación intermedia y una función de activación de salida y los parámetros y representan los parámetros de la red neuronal en cada capa, tendríamos la siguiente expresión:
...
Cuando pensamos en un buscador de imágenes nos podemos hacer a la idea que el buscar al igual que con el texto nos va a mostrar entradas que sean imágenes parecidas a la que estamos buscando.
...
Se recomienda usar el optimizador **ADAM** [@kingma2017adammethodstochasticoptimization] que es el que mejores resultados ha dado en las pruebas según (Walia 2017), consiguiendo una convergencia más rápida que con el resto de optimizadores.
...
En este caso tendremos una capa de **entrada** con **784 neuronas** (correspondientes a los 28x28 pixels de cada imagen), una capa **intermedia** de **32 neuronas**, y una capa de salida de nuevo de las 784 neuronas para poder volver a obtener de nuevo los datos originales.
...
# Tamaño de salida del Encoder
encoding_dim = 32  # 32 floats -> tamaño de compresión 24.5, teniendo una entrada de 784

# Esta es el input
input_img = keras.Input(shape=(784,))
# "encoded" es la representación codificada del input
encoded = layers.Dense(encoding_dim, activation='relu')(input_img)
# "decoded" es la representación decodificada del input
...
# Esta es la entrada codificada (tamaño 32)
encoded_input = keras.Input(shape=(encoding_dim,))
# Cogemos la ultima capa del autoencoder
...
# Codificamos y decodificamos algunos dígitos
# Darse cuenta que toamos los datos de test,
# que no han sido usados en el entrenamiento
...
n = 10  # vamos a mostrar 10 dígitos
...
# TEste modelo mapea una entrada a su reconstrucción
autoencoder = keras.Model(input_img, decoded)
...
# en este putno la representación es (4,4,8) es decir, 128-dimensional
...
Al estar trabajando con imágenes vamos a partir del Autoencoder de Convolución para poder aplicar el denoising.
...
# En este punto la representación es (4, 4, 8) es decir, 128-dimensional
